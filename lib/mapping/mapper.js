...
Mapper.prototype.containsOne = function(mapper, prop) {
  this.on('built', function mapInlineToModel(model, doc) {
    model[prop] = mapper.toModel(doc[prop]);
  });

  this.on('saving', function mapInlineToDoc(model, doc) {
    doc[prop] = mapper.toDoc(model[prop]);
  });
};
...
Mapper.prototype.containsMany = function(mapper, prop) {
  this.on('built', function mapInlineToModel(model, doc) {
    model[prop] = doc[prop].map(mapper.toModel.bind(mapper));
  });

  this.on('saving', function mapInlineToDoc(model, doc) {
    doc[prop] = model[prop].map(mapper.toDoc.bind(mapper));
  });
};
